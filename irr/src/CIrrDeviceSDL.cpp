...
// Since SDL doesn't have mouse keys as keycodes we need to fall back to
// EKEY_CODE in some cases.
...
#ifdef _IRR_EMSCRIPTEN_PLATFORM_
EM_BOOL CIrrDeviceSDL::MouseUpDownCallback(int eventType,
                                           const EmscriptenMouseEvent *event,
                                           void *userData) {
  // We need this callback so far only because otherwise
  // "emscripten_request_pointerlock" calls will fail as their request are
  // infinitely deferred. Not exactly certain why, maybe SDL does catch those
  // mouse-events otherwise and not pass them on.
  return EM_FALSE;
}

EM_BOOL CIrrDeviceSDL::MouseEnterCallback(
  ...
  This->MouseXRel = mouseEvent->movementX; // should be 0 I guess? Or can it
                                           // enter while pointer is locked()?
  This->MouseYRel = mouseEvent->movementY;
  irrevent.MouseInput.ButtonStates =
      This->MouseButtonStates; // TODO: not correct, but couldn't figure out the
                               // bitset of mouseEvent->buttons yet.
...
EM_BOOL CIrrDeviceSDL::MouseLeaveCallback(
    int eventType, const EmscriptenMouseEvent *mouseEvent, void *userData) {
  CIrrDeviceSDL *This = static_cast<CIrrDeviceSDL *>(userData);
...
wchar_t CIrrDeviceSDL::findCharToPassToIrrlicht(uint32_t sdlKey,
                                                EKEY_CODE irrlichtKey,
                                                u16 keymod) {
  switch (irrlichtKey) {
...
std::variant<u32, EKEY_CODE>
CIrrDeviceSDL::getScancodeFromKey(const Keycode &key) const {
  u32 keynum = 0;
  if (const auto *keycode = std::get_if<EKEY_CODE>(&key)) {
    // Fake keys (e.g. mouse buttons): use EKEY_CODE since there is no
...
Keycode CIrrDeviceSDL::getKeyFromScancode(const u32 scancode) const {
  ...
  auto keycode =
      SDL_GetKeyFromScancode((SDL_Scancode)scancode, SDL_KMOD_NONE, true);
#else
  // Modifiers not supported
...
CIrrDeviceSDL::CIrrDeviceSDL(const SIrrlichtCreationParameters &param)
    : CIrrDeviceStub(param), Window((SDL_Window *)param.WindowId), MouseX(0),
      MouseY(0), MouseXRel(0), MouseYRel(0), MouseButtonStates(0),
      Width(param.WindowSize.Width), Height(param.WindowSize.Height),
      Resizable(param.WindowResizable == 1 ? true : false),
      CurrentTouchCount(0), IsInBackground(false) {
...
bool CIrrDeviceSDL::createWindow() {
  ...
  if (CreationParams.DriverDebug) {
    ...
    if (createWindowWithContext()) {
      ...
      // Turn it back on because the GL driver can maybe still do something
      // useful.
...
bool CIrrDeviceSDL::createWindowWithContext() {
  ...
  SDL_GL_SetAttribute(SDL_GL_ALPHA_SIZE,
                      CreationParams.WithAlphaChannel ? 8 : 0);

  SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, CreationParams.ZBufferBits);
  SDL_GL_SetAttribute(SDL_GL_STENCIL_SIZE,
                      CreationParams.Stencilbuffer ? 8 : 0);
  ...
  SDL_CreateWindowAndRenderer(0, 0, SDL_Flags, &Window,
                              &Renderer); // 0,0 will use the canvas size

  logAttributes();

  // "#canvas" is for the opengl context
  emscripten_set_mousedown_callback("#canvas", (void *)this, true,
                                    MouseUpDownCallback);
  emscripten_set_mouseup_callback("#canvas", (void *)this, true,
                                  MouseUpDownCallback);
  emscripten_set_mouseenter_callback("#canvas", (void *)this, false,
                                     MouseEnterCallback);
  emscripten_set_mouseleave_callback("#canvas", (void *)this, false,
                                     MouseLeaveCallback);
  ...
  if (CreationParams.Bits == 16) {
    ...
    SDL_GL_SetAttribute(SDL_GL_ALPHA_SIZE,
                        CreationParams.WithAlphaChannel ? 1 : 0);
  } else {
    SDL_GL_SetAttribute(SDL_GL_RED_SIZE, 8);
    SDL_GL_SetAttribute(SDL_GL_GREEN_SIZE, 8);
    SDL_GL_SetAttribute(SDL_GL_BLUE_SIZE, 8);
  ...
  if (ScaleX != 1.0f || ScaleY != 1.0f) {
    ...
    SDL_SetWindowSize(
        Window, static_cast<int>(CreationParams.WindowSize.Width / ScaleX),
...
void CIrrDeviceSDL::createDriver() {
  if (CreationParams.DriverType == video::EDT_NULL) {
    VideoDriver =
        video::createNullDriver(FileSystem, CreationParams.WindowSize);
    return;
  }
  ...
  case video::EDT_OPENGL:
    VideoDriver =
        video::createOpenGLDriver(CreationParams, FileSystem, ContextManager);
    break;
  case video::EDT_OPENGL3:
    VideoDriver =
        video::createOpenGL3Driver(CreationParams, FileSystem, ContextManager);
    break;
  case video::EDT_OGLES2:
    VideoDriver =
        video::createOGLES2Driver(CreationParams, FileSystem, ContextManager);
    break;
  case video::EDT_WEBGL1:
    VideoDriver =
        video::createWebGL1Driver(CreationParams, FileSystem, ContextManager);
    break;
  default:;
  }
  if (!VideoDriver)
...
static int wrap_PollEvent(SDL_Event *ev) {
  ...
  if (d >= 5) {
    auto msg =
        std::string("SDL_PollEvent took too long: ") + std::to_string(d) + "ms";
...
bool CIrrDeviceSDL::run() {
  ...
  while (!Close && wrap_PollEvent(&SDL_event)) {
    // os::Printer::log("event: ", core::stringc((int)SDL_event.type).c_str(),
    // ELL_INFORMATION);	// just for debugging
    irrevent = {};

    // Initially, we assume that there is no other applicable event type.
    irrevent.EventType = EET_OTHER_SDL_EVENT;
    ...
    case SDL_EVENT_MOUSE_BUTTON_UP: {
      ...
      // For security reasons this will be delayed until the next mouse-up
      // event. We do not pass on this event as we don't want the activation
      // click to do anything.
      if (SDL_event.type == SDL_MOUSEBUTTONDOWN && !isFullscreen()) {
        EmscriptenPointerlockChangeEvent
            pointerlockStatus; // let's hope that test is not expensive ...
        if (emscripten_get_pointerlock_status(&pointerlockStatus) ==
            EMSCRIPTEN_RESULT_SUCCESS) {
          if (CursorControl->isVisible() && pointerlockStatus.isActive) {
            emscripten_exit_pointerlock();
            return !Close;
          } else if (!CursorControl->isVisible() &&
                     !pointerlockStatus.isActive) {
      ...
      // Since Irrlicht does not have event types for X1/X2 buttons, we simply
      // pass those as keycodes instead. This is relatively hacky but avoids the
      // effort of adding more mouse events that will be discarded anyway once
      // we switch to SDL
      case SDL_BUTTON_X1:
      ...
      if (irrevent.EventType == EET_MOUSE_INPUT_EVENT &&
        ...
        if (irrevent.MouseInput.Event >= EMIE_LMOUSE_PRESSED_DOWN &&
            irrevent.MouseInput.Event <= EMIE_MMOUSE_PRESSED_DOWN) {
          u32 clicks = checkSuccessiveClicks(irrevent.MouseInput.X,
    ...
    case SDL_EVENT_KEY_UP: {
      ...
      if (!Keycode::isValid(key))
        os::Printer::log("keycode not mapped", core::stringc(keysym),
                         ELL_DEBUG);
      ...
      if (SDL_TextInputActive(Window) && !keyIsKnownSpecial(key) &&
...
#ifdef _IRR_USE_SDL3_
      {
        ...
        SDL_Keycode keycode_Aa =
            SDL_GetKeyFromScancode(scancode, keymod, false);
        // Discard non-printable, such as CTRL, HOME, F3, ...
        irrevent.KeyInput.Char =
            keycode_Aa < SDLK_EXTENDED_MASK ? keycode_Aa : 0;
      }
#else
    ...
    case SDL_EVENT_USER:
      ...
      irrevent.UserEvent.UserData1 =
          reinterpret_cast<uintptr_t>(SDL_event.user.data1);
      irrevent.UserEvent.UserData2 =
          reinterpret_cast<uintptr_t>(SDL_event.user.data2);
      break;
    ...
    case SDL_EVENT_FINGER_UP:
      ...
      // SDL2: Contrary to what the SDL documentation says,
      // SDL_APP_WILLENTERBACKGROUND and SDL_APP_WILLENTERFOREGROUND are
      // actually sent in onStop/onStart, not onPause/onResume, on recent
      // Android versions. This can be verified by testing or by looking at the
      // org.libsdl.app.SDLActivity Java code.
    ...
    case SDL_EVENT_RENDER_TARGETS_RESET:
      os::Printer::log(
          "Received SDL_RENDER_TARGETS_RESET. Rendering is probably broken.",
          ELL_ERROR);
      break;

    case SDL_EVENT_RENDER_DEVICE_LOST:
      os::Printer::log(
          "Received SDL_RENDER_DEVICE_RESET. Rendering is probably broken.",
          ELL_ERROR);
      break;
  ...
  for (u32 i = 0; i < Joysticks.size(); ++i) {
    ...
    if (joystick) {
      ...
      const int numButtons =
          core::min_(SDL_GetNumJoystickButtons(joystick), 32);
      ...
      const int numAxes =
          core::min_(SDL_GetNumJoystickAxes(joystick),
...
bool CIrrDeviceSDL::activateJoysticks(
  ...
  for (joystick = 0; joystick < (int)joystickInfo.size(); ++joystick) {
    char logString[256];
    snprintf_irr(logString, sizeof(logString),
                 "Found joystick %d, %d axes, %d buttons '%s'", joystick,
                 joystickInfo[joystick].Axes, joystickInfo[joystick].Buttons,
...
std::string CIrrDeviceSDL::getVersionString() const {
  ...
  snprintf_irr(buf, sizeof(buf), "%d.%d.%d (%d.%d.%d)",
               ...
               SDL_VERSIONNUM_MAJOR(ver), SDL_VERSIONNUM_MINOR(ver),
  ...
  SDL_version ver{};
  SDL_GetVersion(&ver);
  snprintf_irr(buf, sizeof(buf), "%d.%d.%d%s", ver.major, ver.minor, ver.patch,
               // the SDL team seems to intentionally number sdl2-compat this
               // way: <https://github.com/libsdl-org/sdl2-compat/tags>
               ver.patch >= 50 ? " (compat)" : "");
  return std::string(buf);
#endif
}
...
void CIrrDeviceSDL::SwapWindow() { SDL_GL_SwapWindow(Window); }

//! pause execution temporarily
void CIrrDeviceSDL::yield() { SDL_Delay(0); }
...
bool CIrrDeviceSDL::setWindowIcon(const video::IImage *img) {
  ...
  SDL_Surface *surface =
#ifdef _IRR_USE_SDL3_
      SDL_CreateSurface(width, height,
                        SDL_GetPixelFormatForMasks(32, 0x00ff0000, 0x0000ff00,
  ...
  bool succ = img->copyToNoScaling(surface->pixels, width, height,
                                   video::ECF_A8R8G8B8, surface->pitch);
  SDL_UnlockSurface(surface);

  if (!succ) {
    os::Printer::log(
        "Could not copy icon image. Is the format not ECF_A8R8G8B8?",
        ELL_ERROR);
...
void CIrrDeviceSDL::closeDevice() { Close = true; }
...
bool CIrrDeviceSDL::setFullscreen(bool fullscreen) {
  ...
  // relevant to us.
  bool success =
      SDL_SetWindowFullscreen(Window, getFullscreenFlag(fullscreen)) == 0;
#endif
  if (!success)
    os::Printer::log("SDL_SetWindowFullscreen failed", SDL_GetError(),
                     ELL_ERROR);
  return success;
}

bool CIrrDeviceSDL::isWindowVisible() const { return !IsInBackground; }

//! Checks if the Irrlicht device supports touch events.
bool CIrrDeviceSDL::supportsTouchEvents() const { return true; }

//! returns if window is active. if not, nothing need to be drawn
bool CIrrDeviceSDL::isWindowActive() const {
#ifdef _IRR_EMSCRIPTEN_PLATFORM_
  // Hidden test only does something in some browsers (when tab in background or
  // window is minimized) In other browsers code automatically doesn't seem to
  // be called anymore.
  EmscriptenVisibilityChangeEvent emVisibility;
  if (emscripten_get_visibility_status(&emVisibility) ==
      EMSCRIPTEN_RESULT_SUCCESS) {
